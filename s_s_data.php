  ...
  <?php
   ...
   $query=mysqli_query($conn,"INSERT INTO stu_data (stu_id,first_name, sex, email, phone, faculty,department,stream,year,image, ac_year,status,ec) VALUES ('$sd','$fn','$sx', '$em', '$ph','$f','$dep','$strm','$y','$image',NOW(),'1',NOW())") ;
   if($query){
  ...
  <?php
   ...
   if(mysqli_num_rows($run)>0)
   {
      ...
      echo"<script>window.open('staff_admin_support.php','_self')</script>";
   }
   else{
   if(getimagesize($_FILES['image']['tmp_name'])==false){
   echo "<script>alert('Please fill photo of Staff')</script>";
   }
   else{
   $image=addslashes($_FILES['image']['tmp_name']);
   $name=addslashes($_FILES['image']['name']);
   $image=file_get_contents($image);
   $image=base64_encode($image);
   $query=mysqli_query($conn,"INSERT INTO staff_table (stu_id,first_name, sex,email, phone, job_level, faculty,department,stream,image,ac_year, status, ec) VALUES ('$sd','$fn','$sx','$em','$ph', '$job', '$f','$dep','$strm','$image', NOW(), '1', NOW())") ;
   if($query){
    echo"<body ><br><h3 style='color:cyan;'><b><script> alert ('The Staff is Registered successfully')</script></b></h3></body>";
    echo"<script>window.open('staff_admin_support.php','_self')</script>";
   }
   else
   echo"<body ><br><h3 style='color:cyan;'><b><script> alert ('Invalid Query')</script></b></h3></body>";
    echo"<script>window.open('staff_admin_support.php','_self')</script>";
    }
   }
   }
    ...
    if($run_query)
    {
      while($row=mysqli_fetch_assoc($run_query)){
        ...
        $em=$row['email'];
         ...
         $strm=$row['stream'];
        $img=$row['image'];
$save="INSERT into non_cafe(stu_id, first_name, sex, email, phone, faculty, department, stream, image, ac_year, status, payment, payment_date) VALUES ('$sd', '$s_name', '$s', '$em','$ph', '$fac', '$dep', '$strm', '$img', NOW(), '$stat','$py', NOW())";
 $result=mysqli_query($conn,$save);   
 if($result){
        echo "<script>alert('Data saved sucess fully!')</script>";
     echo"<script>window.open('dtu_cafe_adminpage.php','_self')</script>";
